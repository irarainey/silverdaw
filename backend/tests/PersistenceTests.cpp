...
void testProjectFileSaveLoadAndViewState()
{
    ...
    const auto viewStateResult = silverdaw::ProjectFile::saveViewState(file, -10.0, 240.0, 99.0, "t1", true, true);
    ...
    // Turning the metronome off via the targeted view-state write must round-trip as off.
    const auto viewStateOff = silverdaw::ProjectFile::saveViewState(file, 0.0, 240.0, 99.0, "t1", true, false);
