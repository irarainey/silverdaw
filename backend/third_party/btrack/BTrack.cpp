...
//=======================================================================
// Local Silverdaw patches for MSVC compatibility (see backend/third_party/btrack/PATCHES.md):
//   1. Define _USE_MATH_DEFINES so <cmath> exposes M_PI.
//   2. Replace several variable-length arrays with std::vector — VLAs are
//      a GCC extension, not standard C++, and MSVC rejects them.
//=======================================================================
...
{
	float output[512];
    std::vector<float> input(static_cast<size_t>(onsetDFBufferSize));
...
void BTrack::adaptiveThreshold (std::vector<double>& x)
{
	...
	std::vector<double> threshold(static_cast<size_t>(N));
...
void BTrack::updateCumulativeScore (double onsetDetectionFunctionSample)
{
	...
	std::vector<double> logGaussianTransitionWeighting(static_cast<size_t>(windowSize));
...
{	 
	...
	std::vector<double> futureCumulativeScore(static_cast<size_t>(onsetDFBufferSize + beatExpectationWindowSize));
	std::vector<double> beatExpectationWindow(static_cast<size_t>(beatExpectationWindowSize));
	...
	std::vector<double> logGaussianTransitionWeighting(static_cast<size_t>(pastWindowSize));
