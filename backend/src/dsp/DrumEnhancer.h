...
// Offline post-separation cleanup for the *drums* stem. Like VocalEnhancer this
// runs on a worker thread after htdemucs-ft has produced (and denormalised) a
// drum buffer and before it is written to disk — it is NOT real-time code, so it
// may allocate and make multiple passes. It is deliberately conservative and
// subtractive: a subsonic high-pass (DC/rumble only — never a musical drum
// high-pass, the kick lives at 40-80 Hz) followed by a soft, range-limited
// downward EXPANDER that pulls down low-level bleed (cymbal wash, vocal/bass
// leakage) in the gaps between hits without ever hard-gating the tails.
//
// Two design choices set it apart from the vocal expander, both to suit
...
//
// Transient shaping, loudness normalisation, multiband/spectral processing and
// any peak-increasing "enhancement" are intentionally out of scope here.
