...
<template>
  <div class="pointer-events-none fixed right-4 top-4 z-50 flex max-w-sm flex-col gap-2" aria-live="polite"
    aria-atomic="false">
    <transition-group name="toast">
      <div v-for="n in notifications.items" :key="n.id"
        ...
        <span class="mt-1 inline-block h-2 w-2 shrink-0 rounded-full"
          :class="n.kind === 'error' ? 'bg-red-500' : 'bg-blue-500'" aria-hidden="true" />
        <span class="flex-1 break-words text-xs leading-snug">{{ n.message }}</span>
        <button type="button"
          class="ml-1 -mr-1 -mt-0.5 shrink-0 rounded p-0.5 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-100"
          title="Dismiss" @click="notifications.dismiss(n.id)">
