...
//   dataTransfer.setData('application/x-jackdaw-library-item', itemId)
...
async function onImportClick(): Promise<void> {
    const opened = await window.jackdaw.openAudioFiles().catch((err) => {
        console.error('[LibraryPanel] openAudioFiles failed:', err)
        return [] as Awaited<ReturnType<typeof window.jackdaw.openAudioFiles>>
    })
...
async function onPanelDrop(e: DragEvent): Promise<void> {
    ...
    for (const file of files) {
        ...
        const path = window.jackdaw.getPathForFile(file)
        if (!path) {
            console.warn('[LibraryPanel] dropped file has no path:', file.name)
            library.noteImportFinished()
            continue
        }
        const opened = await window.jackdaw.readAudioFile(path)
        if (!opened) {
            library.noteImportFinished()
            continue
        }
...
function onItemDragStart(e: DragEvent, item: LibraryItem): void {
    ...
    e.dataTransfer.setData('application/x-jackdaw-library-item', item.id)
