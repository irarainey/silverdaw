...
describe('cropPlatterLane', () => {
  ...
  it('crops and rebases time to zero', () => {
    const result = cropPlatterLane(platter, 500_000, 1_500_000)
    expect(result[0]!.timeUs).toBe(0)
    expect(result[result.length - 1]!.timeUs).toBe(1_000_000)
  })

  it('evaluates boundary values via interpolation', () => {
    const result = cropPlatterLane(platter, 250_000, 750_000)
    expect(result[0]!.turns).toBeCloseTo(0.25, 6)
    expect(result[result.length - 1]!.turns).toBeCloseTo(0.75, 6)
  })

  it('preserves touch state at crop start', () => {
    const result = cropPlatterLane(platter, 500_000, 1_000_000)
    expect(result[0]!.touched).toBe(true)
  })

  it('keeps interior points with rebased time', () => {
    const result = cropPlatterLane(platter, 250_000, 1_500_000)
    const interior = result.slice(1, -1)
    expect(interior.some((k) => k.timeUs === 250_000)).toBe(true) // 500k - 250k
    expect(interior.some((k) => k.timeUs === 750_000)).toBe(true) // 1000k - 250k
  })

  it('produces single point for zero-duration crop', () => {
    const result = cropPlatterLane(platter, 500_000, 500_000)
    expect(result).toHaveLength(1)
    expect(result[0]!.timeUs).toBe(0)
    expect(result[0]!.turns).toBeCloseTo(0.5, 6)
  })
})

describe('cropCrossfaderLane', () => {
  const cf: ScratchCrossfaderKeyframe[] = [
    { timeUs: 0, value: 0 },
    { timeUs: 1_000_000, value: 0.5 },
    { timeUs: 2_000_000, value: 1 }
  ...
  it('crops and rebases', () => {
    const result = cropCrossfaderLane(cf, 500_000, 1_500_000)
    expect(result[0]!.timeUs).toBe(0)
    expect(result[0]!.value).toBeCloseTo(0.25, 6)
    expect(result[result.length - 1]!.timeUs).toBe(1_000_000)
    expect(result[result.length - 1]!.value).toBeCloseTo(0.75, 6)
  })

  it('single point for zero-duration crop', () => {
    const result = cropCrossfaderLane(cf, 1_000_000, 1_000_000)
    expect(result).toHaveLength(1)
    expect(result[0]!.value).toBeCloseTo(0.5, 6)
  })
})

describe('computeCroppedSourceOffset', () => {
  it('adds the platter advance at crop start to original offset', () => {
    const platter: ScratchPlatterKeyframe[] = [
      { timeUs: 0, turns: 0, touched: true },
      { timeUs: 1_000_000, turns: 2, touched: true }
    ]
    const offset = computeCroppedSourceOffset(1.0, platter, 500_000)
    expect(offset).toBeCloseTo(2.0, 6) // 1.0 + (1.0 - 0)
  })

  it('handles negative platter motion', () => {
    const platter: ScratchPlatterKeyframe[] = [
      { timeUs: 0, turns: 2, touched: true },
      { timeUs: 1_000_000, turns: 0, touched: true }
    ]
    const offset = computeCroppedSourceOffset(0, platter, 500_000)
    expect(offset).toBeCloseTo(-1.0, 6) // 0 + (1.0 - 2.0)
  })
})

describe('cropPattern', () => {
  it('produces a valid pattern after crop', () => {
    const p = makePattern()
    const cropped = cropPattern(p, 500_000, 1_500_000)
    expect(cropped).not.toBeNull()
    expect(cropped!.durationUs).toBe(1_000_000)
    expect(cropped!.cropStartUs).toBe(0)
    expect(cropped!.cropEndUs).toBe(1_000_000)
    expect(cropped!.platter[0]!.timeUs).toBe(0)
    expect(cropped!.platter[cropped!.platter.length - 1]!.timeUs).toBe(1_000_000)
    expect(cropped!.crossfader[0]!.timeUs).toBe(0)
    expect(cropped!.crossfader[cropped!.crossfader.length - 1]!.timeUs).toBe(1_000_000)
  })

  it('updates sourceOffsetTurns consistently', () => {
    const p = makePattern({ sourceOffsetTurns: 0.5 })
    const cropped = cropPattern(p, 500_000, 1_500_000)
    expect(cropped).not.toBeNull()
    // At 500k us, platter interpolates to 0.25 turns; offset = 0.5 + (0.25 - 0) = 0.75
    expect(cropped!.sourceOffsetTurns).toBeCloseTo(0.75, 6)
  })

  it('handles zero-duration crop edge case', () => {
    const p = makePattern()
    const cropped = cropPattern(p, 1_000_000, 1_000_000)
    expect(cropped).not.toBeNull()
    expect(cropped!.durationUs).toBe(0)
    expect(cropped!.platter).toHaveLength(1)
    expect(cropped!.crossfader).toHaveLength(1)
  })

  it('returns null for fully invalid crop (beyond duration)', () => {
    const p = makePattern({ durationUs: 1_000_000, platter: [
      { timeUs: 0, turns: 0, touched: true },
      { timeUs: 1_000_000, turns: 1, touched: false }
    ], crossfader: [
      { timeUs: 0, value: 0 },
    ...
    // Crop at exact boundaries should still work
    const cropped = cropPattern(p, 0, 1_000_000)
    expect(cropped).not.toBeNull()
  })

  it('does not mutate source audio — no source fields changed except offset', () => {
    const p = makePattern()
    const cropped = cropPattern(p, 200_000, 1_800_000)
    expect(cropped).not.toBeNull()
    expect(cropped!.id).toBe(p.id)
    expect(cropped!.name).toBe(p.name)
    expect(cropped!.ownerDeck).toBe(p.ownerDeck)
    expect(cropped!.crossfaderCurve).toBe(p.crossfaderCurve)
  })
})
