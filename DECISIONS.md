...
| D-0021 | `IMPORTANT` | Scratch Editor action patterns (Amendment 1: backing accompaniment monitor; Amendment 2: backing durations + monitor trims; Amendment 3: keyboard crossfader cut + library-item authoring; Amendment 4: transport drives the backing channel only; Amendment 5: backing length options 60/120/Full + scratch monitor default 75%; Amendment 6: backing config locks while playing + Clear removed; Amendment 7: crossfader bar colour follows position + MIDI direction; Amendment 8: keyboard crossfader cut is push-to-open from a closed default; Amendment 9: scratch fader bar colour depends on the control source; Amendment 10: MIDI Play/Cue drive the backing bed (ready-gated) + backing timing readout; Amendment 11: default backing length 120s + muted tracks excluded from the bed; Amendment 12: backing loop option (off by default, playback-only); Amendment 13: physical Cue button unbound from the backing bed; Amendment 14: physical Cue button drives scratch recording (mirrors the Record button: arm → record-on-touch → stop); Amendment 15: draft/pattern replay is transport-independent (no backing/pause coupling); Amendment 16: physical Play button drives scratch recording, Cue unbound, backing bed no longer MIDI-driven; Amendment 17: replay position feedback — waveform + notation playheads via `replaying`/`replayPositionNormalized` on the session state) | `docs/adr/0021-scratch-editor-action-patterns.md` |
